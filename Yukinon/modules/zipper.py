...
from Yukinon import TEMP_DOWNLOAD_DIRECTORY
from Yukinon import telethn as client
from Yukinon.events import register
...
async def _(event):
    ...
    if event.is_group:
        if not (await is_register_admin(event.input_chat, event.message.sender_id)):
            await event.reply(
                " Hi.. You are not an admin.. You can't use this command.. But you can use in my pm🙈"
            )
            return
